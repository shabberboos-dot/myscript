...
(function(){
    ...
    loadFirebase(() => {
        ...
        // ৪-স্টেপ জিক-জ্যাক মার্কেট চেক (Big-Small-Big-Small অথবা Small-Big-Small-Big)
        function isZigZag(h) {
        ...
        const UserPatternLogic = (h) => {
            ...
            // ১. ৪-স্টেপ জিক-জ্যাক মার্কেট সনাক্ত হলে রিকভারি মোড অন হবে
            if (!st.isRecovery && isZigZag(h)) {
                st.isRecovery = true;
                console.log(`⚠️ ৪-স্টেপ জিক-জ্যাক মার্কেট সনাক্ত (${h[3] >= 5 ? 'B':'S'}-${h[2] >= 5 ? 'B':'S'}-${h[1] >= 5 ? 'B':'S'}-${h[0] >= 5 ? 'B':'S'}) → RECOVERY MODE ON`);
            }
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        if (isWin) {
                            ...
                            // ৫ নম্বর স্টেপ (Index 4) লস করলে আবার ০ তে ফিরে আসবে এবং রিকভারি/নরমাল মোড অন থাকবে
                            if (st.stpIdx >= 4) {
                                st.stpIdx = 0;
                                console.log("⚠️ ৫ স্টেপ লস! আবার 0 স্টেপ থেকে শুরু হচ্ছে (বর্তমান মোড বজায় থাকবে)।");
                            } else {
                                st.stpIdx = Math.min(st.stpIdx + 1, st.dynSeq.length - 1);
                                console.log("❌ LOSS! বর্তমান মোড চালু থাকবে।");
                            }
                        }
                    }

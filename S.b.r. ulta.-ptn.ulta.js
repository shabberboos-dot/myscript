// ==UserScript==
// @name         𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡 – NORMAL + OPPOSITE + PATTERN RECOVERY (v15.0)
// @namespace    http://tampermonkey.net/
// @version      150.0.0
// @description  NX XHAWON | Normal: Market Follow | 3 Loss → Opposite Trade → Pattern Recovery (Opposite) | Double Martingale | Full Balance on Low
// @author       𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        // ==========================================
        // 3. CUSTOM LOGIC (Normal + Opposite Trade + Pattern Recovery with Opposite)
        // ==========================================
        
        // ---------- NORMAL MODE: MARKET FOLLOW ----------
        function normalModeSignal(lastNumber) {
            if (lastNumber >= 5 && lastNumber <= 9) return 'BIG';
            else if (lastNumber >= 0 && lastNumber <= 4) return 'SMALL';
            else return null;
        }

        // ---------- RECOVERY MODE: 2 PATTERN LOGIC ----------
        ...
        function pattern2Signal(first, last) {
            if (first === 0 || last === 0) return null;
            let sum = first + last;
            if (sum >= 1 && sum <= 4) return 'SMALL';
            else if (sum >= 5 && sum <= 9) return 'BIG';
            else if (sum === 10) return 'SMALL';
            else return null;
        }
        ...
        const UserPatternLogic = (h) => {
            ...
            // ===== NORMAL MODE =====
            if (!st.isRecovery && !st.isOppositeTrade) {
                const sig = normalModeSignal(lastNumber);
                if (sig) {
                    console.log(`✅ NORMAL MODE (FOLLOW): ${lastNumber} → ${sig}`);
                    return sig;
                }
                console.log(`⏳ NORMAL MODE: ${lastNumber} → NO TRADE`);
                return 'NO TRADE';
            }
            
            // ===== OPPOSITE TRADE (৩ লসের পর প্রথম ট্রেড) =====
            if (st.isOppositeTrade) {
                const baseSig = normalModeSignal(lastNumber);
                if (baseSig) {
                    const opposite = (baseSig === 'BIG') ? 'SMALL' : 'BIG';
                    console.log(`🔄 OPPOSITE TRADE: ${lastNumber} → ${baseSig} → OPPOSITE=${opposite}`);
                    return opposite;
                }
                console.log(`⏳ OPPOSITE TRADE: ${lastNumber} → NO TRADE`);
                return 'NO TRADE';
            }
            
            // ===== RECOVERY MODE (2 PATTERN LOGIC + OPPOSITE TRADE) =====
            if (st.isRecovery) {
                const first = h[0];
                const last = h[9];
                
                const p1 = pattern1Signal(h);
                const p2 = pattern2Signal(first, last);
                ...
                if (baseSignal) {
                    const opposite = (baseSignal === 'BIG') ? 'SMALL' : 'BIG';
                    console.log(`🔄 RECOVERY MODE: Base=${baseSignal} → OPPOSITE TRADE=${opposite}`);
                    return opposite;
                } else {
                    console.log(`⏳ RECOVERY MODE: Patterns don't match → NO TRADE`);
                    return 'NO TRADE';
                }
            }
        ...
        // ==========================================
        // 4. 🔥 MARTINGALE CALCULATOR (DOUBLE + FULL BALANCE)
        // ==========================================
        ...
        h.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title">${uF('NX XHAWON')} <span class="custom-badge">REC</span></span><span style="cursor:pointer;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h);
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AI:')}</span><span id="ui-ai" class="txt-blk-reverse">RECOVERY</span></div>`;
        ...
        const updateUI = () => {
            ...
            if (uMode) {
                if (st.isOppositeTrade) {
                    uMode.innerText = uF('OPPOSITE');
                    uMode.className = 'txt-blk-mag';
                    uMode.style.color = '#f0f';
                } else if (st.isRecovery) {
                    uMode.innerText = uF('RECOVERY');
                    uMode.className = 'txt-blk-reverse';
                    uMode.style.color = '#ff6b00';
                } else {
                    uMode.innerText = uF('NORMAL');
                    uMode.className = 'txt-blk-cyan';
                    uMode.style.color = '#0ff';
                }
            }
            if (uLoss) {
            ...
            if (uSignal) {
                uSignal.className = st.isRecovery ? 'txt-blk-reverse' : (st.isOppositeTrade ? 'txt-blk-mag' : 'txt-blk-cyan');
                uSignal.style.color = st.isRecovery ? '#ff6b00' : (st.isOppositeTrade ? '#f0f' : '#0ff');
            }
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        if (isWin) {
                            ...
                            if (st.isOppositeTrade) {
                                st.isOppositeTrade = false;
                                console.log("🎉 OPPOSITE TRADE WIN → NORMAL MODE রিসেট");
                            }
                            ...
                            if (st.isOppositeTrade) {
                                st.isOppositeTrade = false;
                                st.isRecovery = true;
                                console.log(`⚠️ OPPOSITE TRADE LOSS → RECOVERY MODE চালু (2 Pattern Logic)`);
                            }
                            
                            if (st.lossCount >= 3 && !st.isRecovery && !st.isOppositeTrade) {
                                st.isOppositeTrade = true;
                                console.log(`⚠️ ৩টি LOSS → OPPOSITE TRADE চালু`);
                            }
                            
                            if (st.isRecovery) {
                                console.log(`❌ RECOVERY LOSS! Step: ${st.currentStep}`);
                            }
                    ...
                    if (st.lastHist && parseInt(st.lastHist[0]) !== parseInt(sDig[0])) {
                        st.activeAI = st.isRecovery ? 'RECOVERY' : (st.isOppositeTrade ? 'OPPOSITE' : 'NORMAL');

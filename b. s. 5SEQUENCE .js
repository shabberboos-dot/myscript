// ==UserScript==
// @name         𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡 – 5 PATTERN SEQUENCE RECOVERY (v16.0)
// @namespace    http://tampermonkey.net/
// @version      160.0.0
// @description  NX XHAWON | 5 Pattern Detection | 3-8 Step Recovery Sequences | Normal Market Follow | Double Martingale | Full Stats
// @author       𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        // ==========================================
        // 3. 🔥 5 PATTERN SEQUENCES (From Screenshots)
        // ==========================================
        const PATTERN_SEQ_MAP = {
            // প্যাটার্ন ১: SMALL, BIG, SMALL, BIG → 8 Step
            "SMALL-BIG-SMALL-BIG": ["SMALL", "SMALL", "SMALL", "SMALL", "BIG", "BIG", "SMALL", "BIG"],
            // প্যাটার্ন ২: BIG, SMALL, BIG, SMALL → 8 Step
            "BIG-SMALL-BIG-SMALL": ["BIG", "SMALL", "BIG", "BIG", "BIG", "SMALL", "BIG", "SMALL"],
            // প্যাটার্ন ৩: SMALL, SMALL, BIG, BIG → 3 Step
            "SMALL-SMALL-BIG-BIG": ["SMALL", "BIG", "BIG"],
            // প্যাটার্ন ৪: BIG, BIG, SMALL, SMALL → 3 Step
            "BIG-BIG-SMALL-SMALL": ["BIG", "SMALL", "SMALL"],
            // প্যাটার্ন ৫: BIG, SMALL, SMALL, BIG → 3 Step
            "BIG-SMALL-SMALL-BIG": ["BIG", "SMALL", "BIG"]
        };

        function getMarketPattern(last4) {
            let p1 = last4[3] >= 5 ? 'BIG' : 'SMALL';
            let p2 = last4[2] >= 5 ? 'BIG' : 'SMALL';
            let p3 = last4[1] >= 5 ? 'BIG' : 'SMALL';
            let p4 = last4[0] >= 5 ? 'BIG' : 'SMALL';
            let key = p1 + '-' + p2 + '-' + p3 + '-' + p4;
            return PATTERN_SEQ_MAP[key] || null;
        }
        ...
        // ==========================================
        // 5. LOGIC ENGINE (Normal + 5 Pattern Recovery)
        // ==========================================
        const UserPatternLogic = (h) => {
            if (!h || h.length < 4) return 'NO TRADE';
            
            const lastNumber = h[0];
            
            // ===== RECOVERY MODE =====
            if (st.isRecovery) {
                if (st.recoverySequence.length === 0) {
                    let seq = getMarketPattern(h.slice(0, 4));
                    if (seq) {
                        st.recoverySequence = seq;
                        st.recoverySeqIdx = 0;
                        console.log(`🔄 PATTERN FOUND → Recovery Sequence: ${seq.join(' → ')}`);
                        return st.recoverySequence[st.recoverySeqIdx];
                    } else {
                        console.log(`⏳ RECOVERY: No pattern match, skipping...`);
                        return 'NO TRADE';
                    }
                } else {
                    if (st.recoverySeqIdx < st.recoverySequence.length) {
                        let sig = st.recoverySequence[st.recoverySeqIdx];
                        console.log(`🔄 RECOVERY STEP ${st.recoverySeqIdx+1}/${st.recoverySequence.length}: ${sig}`);
                        return sig;
                    } else {
                        st.isRecovery = false;
                        st.recoverySequence = [];
                        st.recoverySeqIdx = 0;
                        console.log(`⚠️ RECOVERY SEQUENCE FINISHED → NORMAL`);
                        return 'NO TRADE';
                    }
                }
            }
        ...
        p.id = 'sys-core-fin';
        p.style.cssText = 'position:fixed;width:230px;padding:4px;font-family:monospace;font-size:10px;z-index:9999999;color:#fff;user-select:none;border-radius:14px;overflow:visible;background:transparent;'; 
        ...
        stl.innerHTML = `
            ...
            .txt-blk-reverse { color: #ff6b00; text-shadow: 1px 1px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 0px 4px 5px #000; font-weight: 900; letter-spacing: 1px; }
            ...
            .custom-badge { display: inline-block; padding: 2px 8px; border-radius: 10px; font-size: 8px; margin-left: 5px; background: #ff6b00; color: #fff; font-weight: bold; }
            .stat-win { color: #00ff88; font-weight: bold; }
            .stat-loss { color: #ff0044; font-weight: bold; }
            .stat-signal { color: #ffcc00; font-weight: bold; }
            .stat-max { color: #ff6b00; font-weight: bold; }
            .stat-super { color: #ffd700; font-weight: bold; text-shadow: 0 0 10px #ffd700; }
        `;
        ...
        h.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title">${uF('NX XHAWON')} <span class="custom-badge">5SEQ</span></span><span style="cursor:pointer;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h);
        ...
        divBtn.onclick = () => { st.mode = 'DIV'; mInpWrap.style.display = 'block'; mInp.placeholder = 'BASE AMOUNT'; mInp.style.color = '#0ff'; dblBtn.style.opacity = '0.4'; divBtn.style.opacity = '1'; };
        dblBtn.onclick = () => { st.mode = 'DBL'; mInpWrap.style.display = 'block'; mInp.placeholder = 'BASE AMOUNT'; mInp.style.color = '#f0f'; divBtn.style.opacity = '0.4'; dblBtn.style.opacity = '1'; };
        ...
        goBtn.innerText = uF('START ENGINE'); goBtn.className = 'txt-blk';
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AI:')}</span><span id="ui-ai" class="txt-blk-reverse">5SEQ</span></div>`;
        ...
        infBx.innerHTML = aiRow +
                            ...
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('STEP:')}</span><span id="ui-step" class="txt-blk-warn" style="color:#ffcc00;">0</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('MODE:')}</span><span id="ui-mode" class="txt-blk-cyan" style="color:#0ff;">NORMAL</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('SIGNAL:')}</span><span id="ui-signal" class="txt-blk-reverse" style="color:#ff6b00;">---</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('TOTAL SIG:')}</span><span id="ui-total-sig" class="stat-signal">0</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('TOTAL WIN:')}</span><span id="ui-total-win" class="stat-win">0</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('TOTAL LOSS:')}</span><span id="ui-total-loss" class="stat-loss">0</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('MAX LOSS:')}</span><span id="ui-max-loss" class="stat-max">0</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('MAX WIN:')}</span><span id="ui-max-win" class="stat-super">0</span></div>` +
                            ...
                            `<div style="display:flex;justify-content:space-between;border-top:2px dashed #000;margin-top:3px;padding-top:3px;"><span class="txt-blk" style="color:#ccc;">${uF('LOSS:')}</span><span id="ui-loss" class="txt-blk-warn" style="color:#ffcc00;">0</span></div>`;
        
        const stpBtn = document.createElement('button');
        stpBtn.innerText = uF('STOP'); stpBtn.className = 'txt-blk-err';
        ...
        p2.appendChild(balBx); p2.appendChild(infBx); p2.appendChild(stpBtn); b.appendChild(p1); b.appendChild(p2); 
        inC.appendChild(b); p.appendChild(inC); document.body.appendChild(p);

        let mlBgDiv = document.createElement('div');
        mlBgDiv.className = 'drx-ml-bg'; mlBgDiv.id = 'ml-bg-engine'; document.body.appendChild(mlBgDiv);
        ...
        const exeTrd = (pred, amt, cb) => {
            ...
            setTimeout(() => {
                ...
                if(inpEl) {
                    ...
                    inpEl.dispatchEvent(new Event('input', { bubbles: true })); inpEl.dispatchEvent(new Event('change', { bubbles: true }));
                }
                setTimeout(() => {
        ...
        const updateUI = () => {
            const uMode = document.getElementById('ui-mode'); const uLoss = document.getElementById('ui-loss');
            const uStep = document.getElementById('ui-step'); const uSignal = document.getElementById('ui-signal');
            const uTotalSig = document.getElementById('ui-total-sig'); const uTotalWin = document.getElementById('ui-total-win');
            const uTotalLoss = document.getElementById('ui-total-loss'); const uMaxLoss = document.getElementById('ui-max-loss');
            const uMaxWin = document.getElementById('ui-max-win');
            
            if (uMode) {
                if (st.isRecovery && st.recoverySequence.length === 0) {
                    uMode.innerText = uF('REC (WAIT)');
                    uMode.className = 'txt-blk-warn'; uMode.style.color = '#ffcc00';
                } else if (st.isRecovery && st.recoverySequence.length > 0) {
                    uMode.innerText = uF(`REC [${st.recoverySeqIdx + 1}/${st.recoverySequence.length}]`);
                    uMode.className = 'txt-blk-reverse'; uMode.style.color = '#ff6b00';
                } else {
                    uMode.innerText = uF('NORMAL');
                    uMode.className = 'txt-blk-cyan'; uMode.style.color = '#0ff';
                }
            }
            if (uLoss) { uLoss.innerText = st.lossCount; uLoss.className = st.lossCount >= 3 ? 'txt-blk-err' : 'txt-blk-warn'; uLoss.style.color = st.lossCount >= 3 ? '#f00' : '#ffcc00'; }
            if (uStep) { uStep.innerText = st.currentStep; uStep.className = st.currentStep >= 7 ? 'txt-blk-err' : 'txt-blk-warn'; uStep.style.color = st.currentStep >= 7 ? '#f00' : '#ffcc00'; }
            if (uSignal) { uSignal.className = st.isRecovery ? 'txt-blk-reverse' : 'txt-blk-cyan'; uSignal.style.color = st.isRecovery ? '#ff6b00' : '#0ff'; }
            if (uTotalSig) uTotalSig.innerText = st.totalSignals; if (uTotalWin) uTotalWin.innerText = st.totalWins;
            if (uTotalLoss) uTotalLoss.innerText = st.totalLosses; if (uMaxLoss) uMaxLoss.innerText = st.maxLossStreak;
        ...
        const loopTask = () => {
            ...
            if (st.curBal >= st.tgtAmt && st.curBal > 0) { uBal.innerText = uF(`${st.curBal.toFixed(2)} (Done)`); } else { uBal.innerText = uF(st.curBal > 0 ? st.curBal.toFixed(2) : '--'); }
            ...
            if(st.curBal >= st.tgtAmt && st.curBal > 0) {
                ...
                dWrap.innerHTML = `<h2 style="color:#a020f0;margin-bottom:10px;font-family:monospace;">✅ TARGET REACHED</h2><p style="color:#fff;font-family:monospace;margin-bottom:5px;">Balance: ${st.curBal.toFixed(2)}</p><button id="closeDWrap" style="background:transparent;color:#a020f0;border:1px solid #a020f0;padding:5px 15px;cursor:pointer;">OK</button>`;
                document.body.appendChild(dWrap); document.getElementById('closeDWrap').onclick = () => dWrap.remove();
                return;
            }

            // ==========================================
            // HISTORY SCANNER & EVALUATION
            // ==========================================
            let sDigRaw = null; let pageText = document.body.innerText;
            let histMatches = [...pageText.matchAll(/(20\d{12,18})[\s\n]+(\d)[\s\n]+(Big|Small)/gi)];
            
            if (histMatches.length >= 10) { sDigRaw = histMatches.map(m => parseInt(m[2])); }
            ...
            if (sDigRaw && sDigRaw.length >= 10) { 
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        st.totalSignals++;
                        if (isWin) {
                            st.totalWins++; st.currentWinStreak++; st.currentLossStreak = 0;
                            if (st.currentWinStreak > st.maxWinStreak) st.maxWinStreak = st.currentWinStreak;
                            st.currentStep = 0; st.stpIdx = 0; st.lossCount = 0;
                            if (st.isRecovery) {
                                st.isRecovery = false; st.recoverySequence = []; st.recoverySeqIdx = 0;
                                console.log("🎉 RECOVERY WIN → NORMAL MODE");
                            }
                        } else {
                            st.totalLosses++; st.currentLossStreak++; st.currentWinStreak = 0;
                            if (st.currentLossStreak > st.maxLossStreak) st.maxLossStreak = st.currentLossStreak;
                            st.lossCount++; st.currentStep++;
                            
                            if (st.isRecovery && st.recoverySequence.length > 0) {
                                st.recoverySeqIdx++;
                                if (st.recoverySeqIdx >= st.recoverySequence.length) {
                                    st.recoverySequence = []; st.recoverySeqIdx = 0;
                                    console.log("⚠️ RECOVERY SEQUENCE FAILED → Waiting for pattern again...");
                                }
                            }

                            if (st.lossCount >= 3 && !st.isRecovery) {
                                st.isRecovery = true; st.recoverySequence = []; st.recoverySeqIdx = 0;
                                console.log(`⚠️ ৩টি LOSS → RECOVERY MODE চালু (Waiting for Pattern)`);
                            }
                        }
                        updateUI();
                    }
                    ...
                    if (st.timeLimit !== 'NO' && parseInt(st.timeLimit) > 0) {
                        if (st.tradesDone >= st.maxTrades && st.curBal < st.tgtAmt) {
                            st.isRun = false; clearInterval(st.autoInt);
                            uSts.innerText = uF('FAIL'); uSts.className = 'txt-blk-err'; lkOvl.style.display = 'none'; document.body.style.overflow = '';
                            ...
                            dWrapF.innerHTML = `<h2 style="color:#f00;margin-bottom:10px;font-family:monospace;">❌ TASK FAILED</h2><button id="closeDWrapF" style="background:transparent;color:#f00;border:1px solid #f00;padding:5px 15px;cursor:pointer;">OK</button>`;
                            document.body.appendChild(dWrapF); document.getElementById('closeDWrapF').onclick = () => dWrapF.remove();
                            return;
                        }
                    }
                    ...
                    if (uSignal) {
                        if (prediction !== 'NO TRADE') {
                            uSignal.innerText = prediction; uSignal.className = st.isRecovery ? 'txt-blk-reverse' : 'txt-blk-cyan'; uSignal.style.color = st.isRecovery ? '#ff6b00' : '#0ff';
                        } else {
                            uSignal.innerText = st.isRecovery ? 'WAIT' : '---'; 
                            uSignal.className = st.isRecovery ? 'txt-blk-warn' : 'txt-blk'; 
                            uSignal.style.color = st.isRecovery ? '#ffcc00' : '#fff';
                        }
                    }

                    if (prediction === 'NO TRADE') {
                        uSts.innerText = st.isRecovery ? uF('PATTERN') : uF('WAIT'); 
                        uSts.className = 'txt-blk-warn';
                        sessionStorage.setItem('drx_sig', cSig); st.lastPred = null; st.lastPeriod = null; return;
                    }
                    ...
                    setTimeout(() => {
                        ...
                        st.dynSeq = calcSeq(nBal, st.tgtAmt); st.stpIdx = 0;
                        let tAmt = st.dynSeq[st.stpIdx]; uBet.innerText = uF(tAmt);

                        if(nBal < tAmt) { uSts.innerText = uF('LOW'); uSts.className = 'txt-blk-err'; st.isTrd = false; return; }

                        uSts.innerText = uF('EXC...'); uSts.className = 'txt-blk'; st.lastPred = prediction;
            ...
            } else { if(!st.isTrd) { uSts.innerText = 'NO DATA'; uSts.className = 'txt-blk-err'; } }
        };

        goBtn.onclick = () => {
            if (window._checkAdminStatus()) return;
            let a = parseFloat(tgtInp.value); if(!a || a <= 0) return;
            st.extVal = parseFloat(mInp.value) || 0; clearInterval(st.preScn);
            
            st.tradesDone = 0; st.isRecovery = false; st.lossCount = 0; st.currentStep = 0;
            st.baseAmount = st.extVal > 0 ? st.extVal : 5;
            st.totalSignals = 0; st.totalWins = 0; st.totalLosses = 0;
            st.maxLossStreak = 0; st.maxWinStreak = 0; st.currentLossStreak = 0; st.currentWinStreak = 0;
            st.recoverySequence = []; st.recoverySeqIdx = 0;
            ...
            scnUI(() => {
                ...
                } else { sessionStorage.setItem('drx_sig', '0-0-0-0-0'); }
                ...
                updateUI(); st.autoInt = setInterval(loopTask, cfg.fRt); 
            });
        };

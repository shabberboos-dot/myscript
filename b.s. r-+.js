// ==UserScript==
// @name         𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡 – NORMAL 1 + RECOVERY (v27.0)
// @namespace    http://tampermonkey.net/
// @version      270.0.0
// @description  NX XHAWON | Normal 1: Market Follow | 3 Loss → Recovery (Pattern Match, Pattern2 = |(1st+2nd)-Last|)
// @author       𝗡𝗫 𝗫𝗛𝗔𝗪𝗢𝗡
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        // ---------- RECOVERY: PATTERN 1 (Pattern of Number) ----------
        ...
        // ---------- RECOVERY: PATTERN 2 (NEW: Absolute Difference) ----------
        function pattern2Signal(history) {
            ...
            // যদি কোনো সংখ্যা 0 হয় তবে স্কিপ (ঐচ্ছিক)
            ...
            const diff = Math.abs(sum - last); // বড় থেকে ছোট বিয়োগ
            console.log(`📊 NEW + PATTERN: |(${first}+${second}) - ${last}| = ${diff}`);
        ...
        const UserPatternLogic = (h) => {
            ...
            if (!st.isRecovery) {
                ...
                if (sig) {
                    console.log(`✅ NORMAL 1 (FOLLOW): ${lastNumber} → ${sig}`);
                    return sig;
                }
                console.log(`⏳ NORMAL 1: ${lastNumber} → NO TRADE`);
                return 'NO TRADE';
            }
            
            // ===== RECOVERY MODE (শুধু প্যাটার্ন ম্যাচ) =====
            if (st.isRecovery) {
        ...
        h.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title">${uF('NX XHAWON')} <span class="custom-badge">N+REC</span></span><span style="cursor:pointer;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h);
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AI:')}</span><span id="ui-ai" class="txt-blk-reverse">N+REC</span></div>`;
        ...
        infBx.innerHTML = aiRow +
                            ...
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AMT:')}</span><span id="ui-bet" class="txt-blk-warn" style="color:#ffcc00;">5</span></div>` +

...
// @version      80.0.4
// @description  Custom Pattern Logic | Firebase Admin Control | Smart Storage | Bug Fixed
...
(function(){
    ...
    loadFirebase(() => {
        ...
        let st = { 
            ...
            isTrd: false, stpIdx: 0, dynSeq: [], mode: 'DEF', extVal: 0,
            mlActive: false, timeLimit: 'NO', tradesDone: 0, maxTrades: 0, activeAI: 'PATTERN 1'
        };
        ...
        // ===================================================
        // HUBOHU USER PATTERN LOGIC (VERIFIED & CORRECTED)
        // ===================================================
        const UserPatternLogic = (h, losses) => {
            if(!h || h.length < 10) return 'WAIT'; 

            let first = parseInt(h[0]); 
            if (isNaN(first)) return 'WAIT'; // Safety Check

            let history = h.slice(0, 10).map(Number);
            let signal1 = 'WAIT';

            // 1st Pattern of Number (১-৯ এবং ০ এর রুলস)
            if (first === 0) {
                // ০ এর ঠিক নিচের সংখ্যাটি (অর্থাৎ আগের পিরিয়ড h[1]) চেক করা হচ্ছে
                let belowNum = parseInt(h[1]); 
                if ([1, 3, 6, 8].includes(belowNum)) {
                    signal1 = 'BIG';
                } else if ([0, 2, 4, 5, 7, 9].includes(belowNum)) {
                    signal1 = 'SMALL';
                }
            } else {
                // ১ থেকে ৯ এর জন্য: ১০টি পিরিয়ডে ওই সংখ্যাটি ১ বারের বেশি না থাকলে
                let count = history.filter(x => x === first).length;
                if (count === 1) { 
                    switch(first) {
                        case 1: case 3: case 4: case 6: case 9: signal1 = 'BIG'; break;
                        case 2: case 5: case 7: case 8: signal1 = 'SMALL'; break;
                    }
                }
            }

            // যদি পর পর ৩ টা লস না হয়, তবে শুধু ১ম প্যাটার্ন অনুযায়ী ট্রেড হবে
            if (losses < 3) return signal1;

            // 2nd Pattern of (PLAS +) -> ৩ টা লস হওয়ার পর একটিভ হবে
            let lastRes = history[9]; // একদম নিচের লাস্ট সংখ্যা
            let sum = first + lastRes;
            let signal2 = 'WAIT';

            // PLAS+ Rules অনুযায়ী যোগফল লজিক
            if (sum >= 0 && sum <= 4) { // ০ এর এরর ফিক্স করা হয়েছে
                signal2 = 'SMALL';
            } else if (sum >= 5 && sum <= 9) {
                signal2 = 'BIG'; 
            } else if (sum === 10) {
            ...
            // যোগফল ১০ এর বেশি হলে signal2 থাকবে 'WAIT' (কোন ট্রেড দিবে না)

            // Final Rule: ২টি প্যাটার্নই মিললে তবেই ট্রেড প্লেস হবে
            if (signal1 !== 'WAIT' && signal1 === signal2) {
                return signal1;
            } else {
                return 'WAIT';
            }
        ...
        let h_ui = document.createElement('div');
        h_ui.style.cssText = 'padding:8px;font-size:12px;display:flex;justify-content:space-between;cursor:move;border-bottom:2px solid #000;background:transparent;';
        h_ui.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title" ondblclick="window._toggleML()">${uF('✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦')}</span><span style="cursor:pointer;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h_ui);

        window._toggleML = function() {
            st.mlActive = !st.mlActive;
            let titleEl = document.getElementById('drx-title');
            if(st.mlActive) {
                titleEl.style.color = '#0ff'; titleEl.innerText = uF('✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦ (ML)');
            } else {
                titleEl.style.color = ''; titleEl.innerText = uF('✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦');
            }
        };
        ...
        h_ui.addEventListener('mousedown',dSt);h_ui.addEventListener('touchstart',dSt,{passive:false});
        ...
        h_ui.querySelector('#sys-cls').onclick = () => { clearInterval(st.autoInt); clearInterval(st.preScn); p.remove(); lkOvl.remove(); document.body.style.overflow = ''; };
        ...
        mInp.type = 'number'; mInp.className = 'txt-blk';
        ...
        let optHtml = "";
        for(let i=1; i<=60; i++) optHtml += `<option value="${i}">${i}</option>`;
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AI:')}</span><span id="ui-ai" class="txt-blk-cyan">PATTERN 1</span></div>`;
            ...
            setTimeout(() => {
                if (PLATFORM_ID === 'deshclub') {
                    ...
                    if (exactMatch && document.querySelector(sel[amt])) {
                        let b_amt = document.querySelector(sel[amt]); if(b_amt) b_amt.click();
                    } else {
            ...
            if (sDigRaw && sDigRaw.length >= 5) {
                ...
                if (cSig !== sSig) {
                    ...
                    setTimeout(() => {
                        ...
                        // ==========================================
                        // EXECUTE FINAL PATTERN LOGIC
                        // ==========================================
                        let prediction = UserPatternLogic(sDig, st.stpIdx);
                        ...
                        if (prediction === 'WAIT') {
                            ...
                            // BUG FIX: Reset last prediction so step doesn't get messed up if WAIT occurs after a loss
                            st.lastPred = null; 

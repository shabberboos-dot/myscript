// ==UserScript==
// @name         ✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦ PRO (CUSTOM PATTERN v8.1 + ADMIN CONTROL)
// @namespace    http://tampermonkey.net/
// @version      81.0.1
// @description  Custom Pattern Logic (NX XHAWON) | Double Trade Fix | Smart Storage
    ...
    loadFirebase(() => {
        ...
        const UserPatternLogic = (h, losses) => {
            ...
            if (first === 0) {
                ...
                if (count === 1) { 
                    if ([1, 3, 4, 6].includes(first)) {
                        signal1 = 'BIG';
                    } else if ([2, 5, 7, 8, 9].includes(first)) {
                        signal1 = 'SMALL';
                    }
                } else if (count > 1) {
                    if ([1, 3, 4, 6].includes(first)) {
            ...
            if (signal1 !== 'WAIT' && signal2 !== 'WAIT' && signal1 === signal2) {
                return signal1;
            } else {
                return 'WAIT';
            }
        ...
        const loopTask = () => {
            ...
            if (sDigRaw && sDigRaw.length >= 5) {
                ...
                // --- FIX: Prevent Double Trading in the same period ---
                if (sessionStorage.getItem('last_traded_period') === cSig) {
                    return; 
                }
                ...
                if (cSig !== sSig) {
                    ...
                    if (isDangerZone) {
                        ...
                        sessionStorage.setItem('last_traded_period', cSig); 
                        return; 
                    }
        ...
        goBtn.onclick = () => {
            ...
            sessionStorage.removeItem('last_traded_period'); 
            st.tradesDone = 0;
            if (st.timeLimit !== 'NO' && parseInt(st.timeLimit) > 0) {

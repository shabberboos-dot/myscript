// ==UserScript==
// @name            CUSTOM RECOVERY LOGIC (v11.0)
// @namespace    http://tampermonkey.net/
// @version      110.0.0
// @description  NX XHAWON | Custom PLUS Logic (6,8,10=SMALL; 5,7,9=BIG) | Normal Market Follow | Unlimited Step Martingale | Full Stats
// @author        
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        let st = { 
            ...
            tradesDone: 0,
            maxTrades: 0,
            activeAI: 'CUSTOM RECOVERY',
            // ===== RECOVERY STATE (maxStep  ) =====
            isRecovery: false,
            lossCount: 0,
            currentStep: 0,
            baseAmount: 5,
        ...
        // ==========================================
        // 3. CUSTOM LOGIC (Normal + Recovery PLUS)
        // ==========================================
        ...
        // ---------- RECOVERY MODE: CUSTOM PLUS PATTERN ----------
        // FIRST + LAST number (skip if either is 0)
        // Sum 6,8,10  SMALL
        // Sum 5,7,9  BIG
        // else  NO TRADE
        function recoveryPlusSignal(first, last) {
            if (first === 0 || last === 0) return null; // skip
            const sum = first + last;
            if (sum === 6 || sum === 8 || sum === 10) return 'SMALL';
            else if (sum === 5 || sum === 7 || sum === 9) return 'BIG';
            else return null; // NO TRADE
        }
        ...
        const UserPatternLogic = (h) => {
            ...
            const tenthNumber = h[9]; // 10th from end (index 9)
            ...
            // ===== RECOVERY MODE =====
            if (st.isRecovery) {
                // Only use custom PLUS pattern
                const sig = recoveryPlusSignal(lastNumber, tenthNumber);
                if (sig) {
                    console.log(` RECOVERY MODE (CUSTOM PLUS): ${lastNumber} + ${tenthNumber} = ${lastNumber+tenthNumber}  ${sig}`);
                    return sig;
                }
                console.log(` RECOVERY MODE: ${lastNumber} + ${tenthNumber} = ${lastNumber+tenthNumber}  NO TRADE`);
                return 'NO TRADE';
            }
        ...
        // ==========================================
        // 4. MARTINGALE CALCULATOR (UNLIMITED STEP)
        // ==========================================
        ...
        h.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title">${uF('NX XHAWON')} <span class="custom-badge">CUSTOM</span></span><span style="cursor:pointer;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h);
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('AI:')}</span><span id="ui-ai" class="txt-blk-reverse">CUSTOM</span></div>`;
        ...
        infBx.innerHTML = aiRow +
                            ...
                            `<div style="display:flex;justify-content:space-between;border-top:2px dashed #000;margin-top:3px;padding-top:3px;"><span class="txt-blk" style="color:#ccc;">${uF('MODE:')}</span><span id="ui-mode" class="txt-blk-cyan" style="color:#0ff;">NORMAL</span></div>` +
                            `<div style="display:flex;justify-content:space-between;"><span class="txt-blk" style="color:#ccc;">${uF('LOSS:')}</span><span id="ui-loss" class="txt-blk-warn" style="color:#ffcc00;">0</span></div>`;
        ...
        const updateUI = () => {
            ...
            if (uMode) {
                uMode.innerText = st.isRecovery ? uF('RECOVERY') : uF('NORMAL');
                uMode.className = st.isRecovery ? 'txt-blk-reverse' : 'txt-blk-cyan';
                uMode.style.color = st.isRecovery ? '#ff6b00' : '#0ff';
            }
            if (uLoss) {
            ...
            if (uSignal) {
                uSignal.className = st.isRecovery ? 'txt-blk-reverse' : 'txt-blk-cyan';
                uSignal.style.color = st.isRecovery ? '#ff6b00' : '#0ff';
            }
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        if (isWin) {
                            ...
                            // WIN  step 0, lossCount reset, recovery off
                            st.currentStep = 0;
                            st.stpIdx = 0;
                            st.lossCount = 0;
                            if (st.isRecovery) {
                            ...
                            // If lossCount >= 3, switch to recovery mode
                            if (st.lossCount >= 3 && !st.isRecovery) {
                                st.isRecovery = true;
                                console.log(`  LOSS  RECOVERY MODE  (Custom PLUS Logic)`);
                            }
                        }
                        updateUI();
                    }

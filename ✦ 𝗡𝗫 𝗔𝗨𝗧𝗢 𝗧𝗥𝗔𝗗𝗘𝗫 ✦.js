...
(function(){
    ...
    loadFirebase(() => {
        ...
        // ==========================================
        // 3. NEW CUSTOM PATTERN LOGIC (NX XHAWON SYSTEM)
        // ==========================================
        const UserPatternLogic = (h) => {
            ...
            let pattern1Signal = 'NO TRADE';
            let pattern2Signal = 'NO TRADE';

            // --- 1ST PATTERN OF NUMBER ---
            if (first === 0) {
                let next = history[1]; 
                if ([1, 3, 6, 8].includes(next)) {
                    pattern1Signal = 'BIG';
                } else if ([0, 2, 4, 5, 7, 9].includes(next)) {
                    pattern1Signal = 'SMALL';
                }
            } else {
                let count = history.filter(x => x === first).length;

                if (count <= 1) {
                    if ([1, 3, 4, 6].includes(first)) pattern1Signal = 'BIG';
                    else if ([2, 5, 7, 8, 9].includes(first)) pattern1Signal = 'SMALL';
                } else {
                    if ([1, 3, 4, 6].includes(first)) pattern1Signal = 'SMALL';
                    else if ([2, 5, 7, 8, 9].includes(first)) pattern1Signal = 'BIG';
                }
            }

            // --- 2ND PATTERN OF (PLUS +) ---
            if (first === 0 || last === 0) {
                pattern2Signal = 'NO TRADE';
            } else {
                let sum = first + last; 

                if (sum >= 1 && sum <= 4) {
                    pattern2Signal = 'SMALL';
                } else if (sum >= 5 && sum <= 9) {
                    pattern2Signal = 'BIG';
                } else if (sum === 10) {
                    pattern2Signal = 'SMALL';
            ...
            // --- ALL TIME TRADE MOD ---
            if (pattern1Signal === 'NO TRADE' || pattern2Signal === 'NO TRADE') {
                return 'NO TRADE';
            } else if (pattern1Signal === 'BIG' && pattern2Signal === 'BIG') {
                return 'BIG';
            } else if (pattern1Signal === 'SMALL' && pattern2Signal === 'SMALL') {
                return 'SMALL';
            } else {
                return 'NO TRADE';
            }
        ...
        const loopTask = () => {
            ...
            // ==========================================
            // NEW: ROBUST HISTORY SCANNER (Regex DOM Parsing)
            // ==========================================
            let sDigRaw = null;
            let pageText = document.body.innerText;
            // Matches format: Period (20...) + space + Number + space + Big/Small
            ...
            if (sDigRaw && sDigRaw.length >= 10) { 
                ...
                // If scanner fails to find history
                if(!st.isTrd) {
        ...
        goBtn.onclick = () => {
            ...
            scnUI(() => {
                ...
                if (hMatches.length >= 5) {
                    ...
                    sessionStorage.setItem('drx_sig', '0-0-0-0-0'); // force start if empty
                }

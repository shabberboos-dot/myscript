// ==UserScript==
// @name         ✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦ PRO (CUSTOM PATTERN v8.5 + FIXED LOSS TRACKER)
// @namespace    http://tampermonkey.net/
// @version      8.5.0
// @description  Custom Advanced Pattern Logic | Firebase Admin Control | Smart Storage
...
(function(){
    ...
    loadFirebase(() => {
        ...
        let st = { 
            ...
            isTrd: false, 
            stpIdx: 0, 
            lossCount: 0, // FIXED: স্বাধীন লস ট্র্যাকিং কাউন্টার
            dynSeq: [], 
            mode: 'DEF', 
            extVal: 0,
            mlActive: false,
        ...
        // ==========================================
        // NEW CUSTOM PATTERN ENGINE (UPDATED v8.5)
        // ==========================================
        const UserPatternLogic = (h) => {
            if(!h || h.length < 10) return 'NO_TRADE'; 

            let first = parseInt(h[0]);
            let next = parseInt(h[1]);
            let tenth = parseInt(h[9]);

            // ১. বেস প্যাটার্ন সিগন্যাল ক্যালকুলেশন (Main / Reverse / Special 0)
            let count = h.slice(0, 10).filter(x => parseInt(x) === first).length;
            let baseSignal = 'NO_TRADE'; 

            if (first === 0) {
                // SPECIAL RULE (যখন ১ম সংখ্যা = 0)
                if ([1, 3].includes(next)) baseSignal = 'BIG';
                else if ([0, 2, 4].includes(next)) baseSignal = 'SMALL';
                else if ([6, 8].includes(next)) baseSignal = 'BIG';
                else if ([5, 7, 9].includes(next)) baseSignal = 'SMALL';
            } else {
                if (count <= 1) {
                    // MAIN PATTERN
                    if ([1, 3, 4, 6].includes(first)) baseSignal = 'BIG';
                    else if ([2, 5, 7, 8, 9].includes(first)) baseSignal = 'SMALL';
                } else {
                    // REVERSE PATTERN
                    if ([1, 3, 4, 6].includes(first)) baseSignal = 'SMALL';
                    else if ([2, 5, 7, 8, 9].includes(first)) baseSignal = 'BIG';
                }
            }

            // ২. মোড ডিটেকশন (পরপর ৩টি LOSS হলে st.lossCount >= 3 হবে)
            let isRecoveryMode = st.lossCount >= 3;
            
            // UI আপডেট
            let uiAi = document.getElementById('ui-ai');
            if (uiAi) {
                uiAi.innerText = uF(isRecoveryMode ? 'RECOVERY' : 'NORMAL');
                uiAi.className = isRecoveryMode ? 'txt-blk-warn' : 'txt-blk-cyan';
            }

            if (!isRecoveryMode) {
                // STEP-1 : NORMAL MODE (Main / Reverse Pattern)
                return baseSignal;
            } else {
                // STEP-2 & 3 : RECOVERY MODE + 2ND PATTERN (PLUS +)
                let sum = first + tenth;
                let plusSignal = 'NO_TRADE';

                if (sum >= 1 && sum <= 4) plusSignal = 'SMALL';
                else if (sum >= 5 && sum <= 9) plusSignal = 'BIG';
                else if (sum === 10) plusSignal = 'SMALL';
                else plusSignal = 'NO_TRADE'; 

                // FINAL CONFIRMATION (দুইটি প্যাটার্নের ফলাফল মিলিয়ে দেখা)
                if (baseSignal === 'BIG' && plusSignal === 'BIG') {
                    return 'BIG';
                } else if (baseSignal === 'SMALL' && plusSignal === 'SMALL') {
                    return 'SMALL';
                } else {
                    return 'NO_TRADE'; // মিল না থাকলে WAIT/NO TRADE
                }
            }
        };
        ...
        window._toggleML = function() {
            ...
            if(st.mlActive) {
                ...
                titleEl.innerText = uF('✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦ (ML)');
            } else {
                titleEl.style.color = '';
                titleEl.innerText = uF('✦ 𝗡𝗫 𝗔𝗨𝗧𝗢 𝗧𝗥𝗔𝗗𝗘𝗫 ✦');
            }
        };
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;"><span class="txt-blk" style="color:#ccc;">${uF('MODE:')}</span><span id="ui-ai" class="txt-blk-cyan">NORMAL</span></div>`;
            ...
            setTimeout(() => {
                if (PLATFORM_ID === 'deshclub') {
                    ...
                    if (exactMatch && document.querySelector(sel[amt])) {
                        let b = document.querySelector(sel[amt]); if(b) b.click();
                    } else {
            ...
            if (sDigRaw && sDigRaw.length >= 10) { 
                ...
                if (cSig !== sSig) {
                    if(st.lastPred && st.lastPeriod) {
                        ...
                        // FIXED: স্বাধীন কাউন্টার ও stpIdx সিঙ্ক
                        if (isWin) {
                            st.stpIdx = 0; 
                            st.lossCount = 0; // উইন হলে লস কাউন্টার রিসেট
                        } else {
                            st.stpIdx = Math.min(st.stpIdx + 1, st.dynSeq.length - 1);
                            st.lossCount++; // লস হলে স্বাধীন কাউন্টার বৃদ্ধি
                        }
                    }
                    ...
                    setTimeout(() => {
                        ...
                        if(nBal < tAmt) { uSts.innerText = uF('LOW'); uSts.className = 'txt-blk-err'; st.stpIdx = 0; st.lossCount = 0; st.isTrd = false; return; }
        ...
        goBtn.onclick = () => {
            ...
            scnUI(() => {
                ...
                chkBal(); st.tgtAmt = a; st.dynSeq = calcSeq(st.curBal, st.tgtAmt); 
                st.stpIdx = 0; st.lossCount = 0; // রিসেট

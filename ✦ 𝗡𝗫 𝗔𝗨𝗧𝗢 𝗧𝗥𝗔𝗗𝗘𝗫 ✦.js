...
// @version      80.0.4
// @description  Custom Pattern Logic | Firebase Admin Control | Smart Storage | Flawless Scanner | Responsive UI
// @author       𝐃𝐑𝐗-𝐓𝐌
// @match        *://*/*
// @grant        none
// ==/UserScript==
...
(function(){
    ...
    loadFirebase(() => {
        ...
        // --- UPDATED FLAWLESS SCANNER LOGIC ---
        function ext(tgt) {
        ...
        // --- UPDATED BALANCE PARSER ---
        function chkBal() {
            let tb = ext(d.B1);
            if(tb.length > 0) {
                let textJoined = tb.join(" ");
                let match = textJoined.match(/[\d,]+(\.\d+)?/);
                if(match) {
        ...
        // --- UI INJECTION & RESPONSIVE LAYOUT ---
        let p = document.createElement('div');
        p.id = 'sys-core-fin';
        // Updated styling for bounds matching and scaling flexibility
        p.style.cssText = 'position:fixed;width:clamp(170px, 45vw, 220px);padding:4px;font-family:monospace;font-size:10px;z-index:9999999;color:#fff;user-select:none;border-radius:14px;overflow:visible;background:transparent;box-sizing:border-box;touch-action:none;'; 
        
        let sL = localStorage.getItem('drx_ui_x');
        let sT = localStorage.getItem('drx_ui_y');
        if(sL && sT) { 
            p.style.left = sL; p.style.top = sT; 
        } else { 
        ...
        stl.innerHTML = `
            ...
            .drx-wrap { box-sizing: border-box; backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); background: rgba(5,5,5,0.85) !important; box-shadow: 0 4px 15px rgba(160,32,240,0.3); transition: transform 0.2s ease; }
            ...
            .drx-in * { box-sizing: border-box; }
            input, select, button { width: 100%; }
            input:focus, select:focus { outline: 1px solid #a020f0 !important; box-shadow: 0 0 5px #a020f0; }
            ...
            @media screen and (max-width: 480px) {
                .drx-wrap { transform: scale(0.95); transform-origin: top right; }
            }
            @media screen and (max-width: 360px) {
                .drx-wrap { transform: scale(0.85); transform-origin: top right; }
            }
        `;
        ...
        h.innerHTML = `<span class="txt-blk drx-title-anim" id="drx-title" ondblclick="window._toggleML()">${uF('𝐃𝐑𝐗-𝐓𝐌')}</span><span style="cursor:pointer;padding:0 5px;" class="txt-blk-err" id="sys-cls">X</span>`;
        inC.appendChild(h);
        ...
        // --- UPDATED DRAG POSITION & BOUNDARY LOGIC ---
        ...
        function dMv(e){
            ...
            let nx = sl+ev.clientX-sx;
            let ny = st_y+ev.clientY-sy;
            
            let maxW = window.innerWidth - p.offsetWidth;
            let maxH = window.innerHeight - p.offsetHeight;
        ...
        // Auto correct position on screen resize/rotate
        window.addEventListener('resize', () => {
            let maxW = window.innerWidth - p.offsetWidth;
            let maxH = window.innerHeight - p.offsetHeight;
            let curX = parseInt(p.style.left) || 0;
            let curY = parseInt(p.style.top) || 0;
            if (curX > maxW) p.style.left = Math.max(0, maxW) + 'px';
            if (curY > maxH) p.style.top = Math.max(0, maxH) + 'px';
        });
        ...
        tgtInp.style.cssText = 'padding:8px;margin-bottom:8px;background:transparent;border:2px solid #000;border-radius:4px;text-align:center;font-size:12px;outline:none;';
        
        const mWrap = document.createElement('div');
        mWrap.style.cssText = 'display:flex;gap:5px;margin-bottom:8px;width:100%;';
        ...
        mInpWrap.style.cssText = 'display:none;margin-bottom:8px;width:100%;';
        const mInp = document.createElement('input');
        mInp.type = 'number';
        mInp.className = 'txt-blk';
        mInp.style.cssText = 'padding:8px;background:transparent;border:2px solid #000;border-radius:4px;text-align:center;font-size:12px;outline:none;';
        ...
        timeWrap.style.cssText = 'margin-bottom:8px;width:100%;';
        const timeSel = document.createElement('select');
        timeSel.className = 'txt-blk';
        timeSel.style.cssText = 'padding:8px;background:rgba(0,0,0,0.8);border:2px solid #000;border-radius:4px;text-align:center;font-size:12px;outline:none;color:#fff;appearance:none;';
        ...
        goBtn.style.cssText = 'padding:8px;background:transparent;border:2px solid #000;border-radius:4px;cursor:pointer;font-size:11px;transition:0.2s;';
        ...
        let aiRow = `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;padding-bottom:2px;margin-bottom:2px;"><span class="txt-blk" style="color:#ccc;">${uF('MD:')}</span><span id="ui-ai" class="txt-blk-cyan">NORMAL</span></div>`;
        ...
        infBx.innerHTML = aiRow +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;padding-bottom:2px;margin-bottom:2px;"><span class="txt-blk" style="color:#ccc;">${uF('TGT:')}</span><span id="ui-tgt" class="txt-blk" style="color:#fff;">0</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;padding-bottom:2px;margin-bottom:2px;"><span class="txt-blk" style="color:#ccc;">${uF('STP:')}</span><span id="ui-bet" class="txt-blk-warn" style="color:#ffcc00;">5</span></div>` +
                            `<div style="display:flex;justify-content:space-between;border-bottom:2px dashed #000;padding-bottom:2px;margin-bottom:2px;"><span class="txt-blk" style="color:#ccc;">${uF('CLK:')}</span><span id="ui-clk" class="txt-blk" style="color:#fff;">${timerPlaceholder}</span></div>` +
        ...
        stpBtn.style.cssText = 'padding:8px;background:transparent;border:2px solid #000;border-radius:4px;cursor:pointer;font-size:11px;margin-top:6px;transition:0.2s;';
        ...
        const loopTask = () => {
            ...
            let sDigRaw = (tmText.match(/\b\d\b/g) || tmText.replace(/[^0-9]/g, '').split(''));
        ...
        goBtn.onclick = () => {
            ...
            scnUI(() => {
                ...
                if (sDigRaw && sDigRaw.length >= 5) sessionStorage.setItem('drx_sig', sDigRaw.map(Number).slice(0, 5).join("-"));
